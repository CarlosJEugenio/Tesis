...
\begin{document}
	...
	\begin{frame}{Introducción}
		\begin{figure}
			...
			\begin{subfigure}[b]{0.2\textwidth}
				\begin{tikzpicture}
				\node (image) {\includegraphics[width=\textwidth]{graphene_structure.pdf}};
...
%				\end{tikzpicture}
	...
	\begin{frame}
		\begin{figure}[h!]
			\centering
			\begin{tikzpicture}
				\node (image) {\includegraphics[width=0.7\textwidth]{edlc_schem.pdf}};
		...
		\begin{figure}
			...
			\path[ use as bounding box] (-5,-5) rectangle(5,3);
				\only<1->{\begin{scope}
					...
					\node at (0, -1) {\includegraphics[width=\textwidth]{experimental_method.pdf}};
				\end{scope}}
				\only<2->{\begin{scope}
		...
		\begin{figure}
			\begin{tikzpicture}
				...
				legend entries={Ideal, Real},				
 	 			...
 	 			\addplot[red, thick] {((x+1)*(2^(6-1)))^(1/6)-1};
 	 			\addplot[red, thick] {(1/2^(6-1))*(x+1)^6-1};
 		 		\end{axis}
			\end{tikzpicture}
		\end{figure}
	\end{frame}
