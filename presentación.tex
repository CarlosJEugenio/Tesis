	...
	\begin{frame}{¿Por qué usar supercondensadores?}
		\begin{columns}
			\begin{column}{0.5\textwidth}
				...
				\begin{itemize}
					\item<2-> Es un dispositivo eléctrico de \textbf{almacenamiento de energía}.
					\item<3-> Parecido a una batería.
					\item<4-> Pero más parecido a un condensador convencional.
					\item<5-> Puede almacenar y entregar energía \textbf{más rápido que una batería}, y de forma \textbf{más eficiente}.
				\end{itemize}
			\end{column}
			\begin{column}{0.5\textwidth}
				\begin{onlyenv}<5->
				...
				\begin{onlyenv}<1-4>
					\begin{figure}
						\begin{tikzpicture}
							...
							\node at (0,0) {\includegraphics[width=0.5\textwidth]{com_sc_2.png}};
							\node at (2,-2) {\includegraphics[width=0.6\textwidth]{com_sc_3.png}};
						\end{tikzpicture}
...
%			\draw [help lines, dashed] (-4,-2) grid (4,2);
				...
				\only<3->{\begin{scope}
					\node[] at (5,2) {\ce{Mn2O7}};
					\node[scale=1] at (5,2.5) {\includegraphics[width=2.5cm]{Mn2O7.pdf}};
...
%			\node[black, rotate=90] at (24, 0.3){\Tiny{(002)}};
	...
	\begin{frame}{Construcción de supercondensadores}
		\begin{itemize}[<+->]
			...
			\item[] \color{red} \textbf{Hay que buscar otro método!}
		\end{itemize}
	\end{frame}
...
%	\end{frame}
	...
	\captionsetup[subfigure]{labelformat=empty}
		\begin{columns}
			...
			\begin{column}{0.6\textwidth}
				\begin{itemize}[<+->]
					\item Lámina parecida al papel.
					\item[!] Bordes irregulares.
					\item Material liofilizado.
	...
	\captionsetup[subfigure]{labelformat=empty}
		\begin{columns}
			...
			\begin{column}{0.6\textwidth}
				\begin{itemize}[<+->]
					\item Dispersión en etilenglicol, deposición por goteo, y evaporación del dispersante.
					\item[!] ?.
					\item Dispersión en solución de ppma en acetona, deposición por goteo, y evaporación del solvente.
					\item[!] ?.
				\end{itemize}
			\end{column}
		\end{columns}		
