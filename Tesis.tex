...
\newcommand{\SCGraphs}[6]{
	\begin{figure}
		...
		\begin{subfigure}{\plotwidth}
			\begin{tikzpicture}[scale=\plotscale,trim axis right,trim axis left]
			\begin{axis}[SCStyle]
			\addplot+ [error bars/.cd, y dir=both, y fixed relative=0.1,] table [only marks] {./Data/#1/raw/capacitance.txt};
			\addplot+ [error bars/.cd, y dir=both, y fixed relative=0.1,] table [only marks] {./Data/#2/raw/capacitance.txt};	
			\end{axis}
			\end{tikzpicture}
		\end{subfigure}\hfill	
		\begin{subfigure}{\plotwidth}
