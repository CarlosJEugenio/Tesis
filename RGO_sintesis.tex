...
En la figura \ref{fig:raman} se muestra el espectro Raman del óxido de grafeno y óxido reducido de grafeno sintetizados.  La proporción entre la intensidad de las bandas D y G ($I_D/I_G$), da cuenta de la presencia de defectos en la red del grafeno \citep{Abdolhosseinzadeh2015, Li2014}. 
\begin{figure}
	\centering
	\begin{tikzpicture}[]
		\begin{axis}[
...
%		\addplot+[restrict x to domain=1000:3500] table [x expr = \thisrow{raman_shift}, y expr=\thisrow{GO090615_1} + 10] {./Data/RAMAN/raman.txt};
		...
		%	\addplot table [x expr = \thisrow{2t}, y expr=\thisrow{RGOLYO}+3] {./Data/XRD/xrd.txt};
		\node[black] at (1350, 290){\small{D-band}};
		\node[black] at (1600, 273){\small{G-band}};
		\node[black] at (2700, 220){\small{2D-band}};
		\end{axis}
	\end{tikzpicture}
	\caption{Espectro Raman de muestras de óxido de grafeno y óxido reducido de grafeno.}
	\label{fig:raman}
\end{figure}
